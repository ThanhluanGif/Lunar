...
|---|---|---|---|---|
| Function | `scanCodeForSecurityVulnerabilities(code, path)` | Client side scanner | `code: string, path: string` | `VulnerabilityScanResult { score, issues, summary }` |
| State Handler | `setActiveTab(tabName)` | UI View Switcher | `tabName: 'explore'|'community'|'detail'` | Updates rendered view component |
| State Handler | `setSelectedProject(project)` | Project Selection | `project: SecurityProject` | Renders Project Detail & Code Viewer |
| State Handler | `handleUpgradeSuccess(tier)` | Subscription Handler | `tier: 'PRO'|'ENTERPRISE'` | Updates user tier & unlocks Pro features |
...
interface SecurityProject {
  ...
  vulnerabilities: Array<{ id: string; title: string; severity: 'critical'|'warning'|'info'; line: number }>;
  communityAudits: Array<{ author: string; comment: string; date: string }>;
}
```
...
- FR1 → `setActiveTab('explore')`, FigmaLunarLanding component render
- FR2 → `setSelectedProject(project)`, CodeRepairWorkbench component render
- FR3 → `scanCodeForSecurityVulnerabilities(code, path)`, SecurityDashboard component render
- FR4 → `handleUpgradeSuccess(tier)`, Modals (AuthModal, PricingModal, GitBotConfigModal, etc.)
